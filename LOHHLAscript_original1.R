...
create.kmer.file <- function(workDir, kmerSize, HLAfastaLoc){
  ...
  system(cmd)
  cmd <- paste('jellyfish dump mer_counts.jf > mer_counts_dumps.fa', sep = '')
  system(cmd)
  cmd <- paste('grep --invert-match \\> mer_counts_dumps.fa > ', kmerSize, 'mer_uniq', sep = '')
  system(cmd)
  system('rm mer_counts.jf')
  system('rm mer_counts_dumps.fa')
...
## 
##   # add header
##   cmd <- paste('samtools view -H ', BAMDir, '/', BAMfile, ' > ', regionDir, '/fished.sam', sep = '')
##   system(cmd)
## 
...
get.partially.matching.reads <- function(workDir, regionDir, BAMfile){
  ...
  # add header
  cmd <- paste('samtools view -H ', BAMfile, ' > ', regionDir, '/fished.sam', sep = '')
  system(cmd)

  # fish partially matching reads
  cmd <- paste('samtools view ', BAMfile, ' | grep -F -f ', kmerFile, ' >> ', regionDir, '/fished.sam', sep = '')
  system(cmd)
...
combine.fastqs <- function(chr6.f1, chr6.f2, fished.f1, fished.f2){
  ...
  fished.f2.seq <- read.table(fished.f2, sep = '\t', stringsAsFactors = FALSE, comment.char = '', quote = '')
  ...
  write.table(new.chr6.f1.seq, file = chr6.f1, sep = '\t', quote = FALSE, row.names = FALSE, col.names = FALSE)
  write.table(new.chr6.f2.seq, file = chr6.f2, sep = '\t', quote = FALSE, row.names = FALSE, col.names = FALSE)
...
params <- list(full.patient, workDir, hlaPath, normalBAMfile, tumorBAMfile, HLAfastaLoc, CopyNumLoc, GATKDir, NOVODir)
names(params) <- c('full.patient', 'workDir', 'hlaPath', 'normalBAMfile', 'tumorBAMfile', 'HLAfastaLoc', 'CopyNumLoc', 'GATKDir', 'NOVODir')
...
## put both tumor and normal BAM here; keeps the same data structures --- probably a bit hacky
...
if(mapping.step){
  ...
  novoindexCMD <- paste(NOVODir, '/novoindex ', workDir, '/', full.patient, '.patient.hlaFasta.nix', ' ', workDir, '/', full.patient, '.patient.hlaFasta.fa', sep = '')
  ...
  for(BAMfile in BAMfiles){
    ...
    samtoolsCMD <- paste("samtools view ", BAMfile, " chr6:29909037-29913661 >> ",regionDir,"/",BAMid,".chr6region.sam",sep="")
    write.table(samtoolsCMD, file = log.name, quote = FALSE, row.names = FALSE, col.names = FALSE, append = TRUE)
    system(samtoolsCMD)
    
    samtoolsCMD <- paste("samtools view ", BAMfile, " chr6:31321649-31324964 >> ",regionDir,"/",BAMid,".chr6region.sam",sep="")
    write.table(samtoolsCMD, file = log.name, quote = FALSE, row.names = FALSE, col.names = FALSE, append = TRUE)
    system(samtoolsCMD)
